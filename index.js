...
hbs.registerHelper("caro", function(items, options) {
  let out = '<div class="carousel-inner" role="listbox">';
...
hbs.registerHelper("checkedIf", function(test) {
  return test ? "checked" : "";
});
hbs.registerHelper("selected", function(sel) {
  return sel ? "selected" : "";
});
          ...
          Object.assign(
            {
              ...
              type: "article",
              description:
                "The Fishwrapper is UW's own satirical newspaper, " +
                "committed to publishing all the news that's unfit to print. " +
                "Irrelevant, irreverent, irresponsible.",
              url: BASE_URL,
              ogImage: process.env.S3_BUCKET + "logo.png"
            },
            obj
          )
        );
...
app.get("/sitemap.xml", function(req, res) {
  ...
  dynamoDb.scan({ TableName: process.env.POSTS_TABLE }, function(err, data) {
    if (err) {
      ...
      dynamoDb.scan({ TableName: process.env.QUIZZES_TABLE }, function(
        er,
        dat
      ) {
        if (er) {
          ...
          for (let i = 0; i < data.Count; i++) {
            ...
            if (!post.staging) {
              out += "<url>";
              out += `<loc>https://thefishwrapper.news/posts/${escape(
                post.postId
              )}</loc>`;
              if (post.thumbnail) {
          ...
          for (let i = 0; i < dat.Count; i++) {
            ...
            out += `<loc>https://thefishwrapper.news/quizzes/${escape(
              quiz.quizId
            )}</loc>`;
            if (quiz.thumbnail) {
